...
// AllReduce operators (bonus 4.4). AVG is not a switch operator: workers
// request SUM and floor-divide the result by `world` locally -- the switch
...
header sml_t {
  ...
  bit<8>  op;      // OP_SUM / OP_MIN / OP_MAX (was the pad byte)
...
// value is combined according to the packet's op field. MIN/MAX compare as
// SIGNED 32-bit values (the cast matters: bit<32> compares unsigned, which
// would order negative values above positive ones). The running result is
// also written back into the header, so the final contribution's packet
// already carries the complete result. All workers of a round use the same
// op (they execute the same sequence of AllReduce calls).
#define AGGREGATE(POOL, FIELD)                                        \
    POOL.read(tmp, idx);                                              \
    if (cnt == 0)                      { tmp = FIELD; }               \
    else if (hdr.sml.op == OP_MIN) {                                  \
      if ((int<32>)FIELD < (int<32>)tmp) { tmp = FIELD; }             \
    } else if (hdr.sml.op == OP_MAX) {                                \
      if ((int<32>)FIELD > (int<32>)tmp) { tmp = FIELD; }             \
    } else                             { tmp = tmp + FIELD; }         \
    POOL.write(idx, tmp);                                             \
    FIELD = tmp;
