...
# Operator byte carried in the header (must match switch.p4). AVG is done
# entirely at the worker: the switch computes SUM and each worker
# floor-divides the result by the world size (matches Test._reduce, which
# uses Python's // -- true floor, also for negatives).
OPS = {"sum": 0, "min": 1, "max": 2, "avg": 0}
...
class MyCollectives(Collectives):
    ...
    # ------------------------------------------------------------------ send
    def _send_chunk(self, gid, vals, opcode):
        slot = gid % WINDOW
        ver = (gid // WINDOW) & 1
        payload = HDR.pack(FLAG_REQ, self.rank, self.world, slot, ver, opcode,
    ...
    def AllReduce(self, input: list[int], output: list[int], op: str = "sum"):
        ...
        opcode = OPS[op.lower()]   # KeyError on unsupported operators
        is_avg = op.lower() == "avg"
        ...
        while ndone < nchunks:
            ...
            while (next_c < nchunks and len(outstanding) < WINDOW
                ...
                self._send_chunk(base + next_c, chunk_vals(next_c), opcode)
            ...
            try:
                ...
                if len(data) == HDR.size:
                    ...
                    if flags == FLAG_RES and 0 <= c < nchunks and not done[c]:
                        lo = c * CHUNK
                        m = min(CHUNK, n - lo)
                        vals = [s32(v) for v in f[7:7 + m]]
                        if is_avg:
                            vals = [v // self.world for v in vals]
                        output[lo:lo + m] = vals
                        done[c] = True
                        ndone += 1
            ...
            for c, t in list(outstanding.items()):
                if now - t > RTO:
                    self._send_chunk(base + c, chunk_vals(c), opcode)
                    outstanding[c] = now
...
if __name__ == "__main__":
    ...
    # Many back-to-back AllReduce calls over all test patterns, ALL four
    # operators (sum/min/max/avg), and a mix of sizes: smaller than a chunk,
    # exact chunk, exact window, non-multiples.
    for size in (1, 3, CHUNK, CHUNK * WINDOW, 66, 250):
        print(f"---- size {size} ----")
        Test.test_allreduce_all(coll, args.rank, args.world, size,
                                ("sum", "min", "max", "avg"))
